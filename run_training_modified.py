...
class TrainConfig:
    ...
    data: (
        ...
        | Annotated[C3VDDataConfig, tyro.conf.subcommand(name="c3vd")]
    )
    lr: SceneLRConfig
    loss: LossesConfig
...
def init_model_from_tracks(
    ...
    bg_params = None
    if num_bg > 0:
        bg_xyz, bg_normals, bg_colors = train_dataset.get_bkgd_points(num_bg)
        if bg_xyz.numel() == 0:
            guru.warning(
                "Dataset returned no background samples; skipping background initialization."
            )
        else:
            bg_points = StaticObservations(bg_xyz, bg_normals, bg_colors)
